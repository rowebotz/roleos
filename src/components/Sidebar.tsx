...
export function Sidebar() {
  ...
  return (
    <div className="flex flex-col h-full border-r bg-zinc-950/50 backdrop-blur-xl w-64" role="tablist" aria-label="Context Taxonomies">
      <div className="p-6 border-b border-white/5 flex items-center justify-between">
        <h2 className="text-sm font-bold tracking-widest text-zinc-400 uppercase">Taxonomies</h2>
        <Settings2 className="w-3 h-3 text-zinc-700" />
      </div>
          ...
          {ROLE_OS_SECTIONS.map((section) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "w-full flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-md transition-all duration-200 group focus:outline-none focus:ring-1 focus:ring-indigo-500/50",
                  isActive
