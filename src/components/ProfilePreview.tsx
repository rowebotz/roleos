...
export function ProfilePreview() {
  ...
  const headline = profile.role?.trim() || 'Your Profile';
  return (
    <div className="flex flex-col h-full w-full md:w-[450px] bg-sidebar md:border-l border-border" aria-label="Live Profile Preview">
      <div className="p-6 border-b border-border flex items-center justify-between bg-sidebar/70 backdrop-blur-sm sticky top-0 z-10">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xs font-bold tracking-[0.2em] text-muted-foreground uppercase">Live Preview</h2>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" aria-hidden="true" />
          <span className="px-2 py-0.5 rounded text-[10px] font-medium bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30 tracking-wide">
            Auto-saved
          </span>
        </div>
      </div>
      <ScrollArea className="flex-1 p-6 md:p-8">
        <div className="space-y-12 font-mono text-xs leading-relaxed max-w-sm mx-auto pb-12">
          <header className="text-center space-y-3">
            <h2 className="text-lg font-bold tracking-tight text-foreground font-sans">{headline}</h2>
            <div className="flex items-center gap-2 justify-center" aria-hidden="true">
              ...
              <div className="text-[10px] text-muted-foreground font-medium uppercase tracking-widest text-center">AI Operating Profile</div>
            ...
            <div className="h-64 flex flex-col items-center justify-center text-center space-y-6 px-4" role="status">
              <div className="relative">
                <div className="w-16 h-16 rounded-full border border-dashed border-border/60 flex items-center justify-center">
                  <Sparkles className="w-6 h-6 text-border" aria-hidden="true" />
                </div>
              ...
              <div className="space-y-2 font-sans">
                <p className="text-foreground/80 font-semibold text-sm">Your profile will appear here</p>
                <p className="text-muted-foreground text-xs">Fill in a section on the left and watch it build in real time.</p>
              </div>
            </div>
          )}
        </div>
      </ScrollArea>
      <footer className="p-4 border-t border-border bg-sidebar/80 text-[10px] text-muted-foreground flex justify-between items-center">
